"""Fetch the Edison cover-letter feedback task result once (no polling loop).

Run from paper/cover_letter_query/:  python3 fetch_results.py
Prints the task status. When the task is done, writes:
  edison_cover_letter_feedback.md  -- the answer/report
  edison_task_response.json   -- full task response for provenance
  artifacts/                  -- files attached to the trajectory
"""
import json
import os
import pathlib
...
def main():
    ...
    if answer:
        with open("edison_cover_letter_feedback.md", "w") as f:
            f.write(str(answer))
        print("wrote edison_cover_letter_feedback.md")
    else:
