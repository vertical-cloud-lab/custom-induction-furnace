"""Fetch the Edison manuscript-review task result once (no polling loop).

Run from paper/manuscript_review_query/:  python3 fetch_results.py
Prints the task status. When the task is done, writes:
  edison_review_report.md  -- the answer/report
...
def main():
    ...
    if answer:
        with open("edison_review_report.md", "w") as f:
            f.write(str(answer))
        print("wrote edison_review_report.md")
    else:
