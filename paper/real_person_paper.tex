...
with a documented, generator-agnostic retrofit. On a modern solid-state
generator, the system demonstrated a power--temperature calibration with
$R^2 = 0.991$, soak temperatures reproducible to 0.11\,\%, and stable
closed-loop soaks up to 40\,h. The anneals it delivers are clean enough
that the nickel specimens needed no metallographic preparation at all. They
went straight from the furnace chamber into the EBSD chamber and returned
high-quality orientation maps with no grinding, polishing, or etching,
while showing the controlled grain growth the system was built to produce.

The same furnace also reaches refractory ceramics without redesign.
Exchanging only the materials in contact with the specimen carries the
system from metal anneals in the graphite crucible to YSZ grain growth at
2500\,\celsius. The trial-and-error survey behind that result identified
chemistry, not power, as the governing constraint at extreme temperature.
Every failure mode was a contact reaction among the charge, the susceptor,
and the support materials. The interchangeable sample assembly lets a user
iterate on those material choices run by run. Because the retrofit requires
only a monotonic analog power-control input, the design transfers across
generators and is directly reusable at a small fraction of turn-key cost.
In the annealed nickel, grains and their boundaries span the full sheet
thickness, and the deep thermal grooves that mark those boundaries point to
columnar grain-growth studies as a natural next
