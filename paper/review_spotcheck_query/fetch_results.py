"""Fetch the Edison spot-check task result once (no polling loop).

Run from paper/review_spotcheck_query/:  python3 fetch_results.py
Prints the task status. When the task is done, writes:
  edison_spotcheck_report.md  -- the answer/report
...
def main():
    ...
    if answer:
        with open("edison_spotcheck_report.md", "w") as f:
            f.write(str(answer))
        print("wrote edison_spotcheck_report.md")
    else:
