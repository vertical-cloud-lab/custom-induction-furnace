"""Fetch the Edison YSZ-references literature task result once (no polling loop).

Run from paper/ysz_refs_query/:  python3 fetch_results.py
Prints the task status. When the task is done, writes:
  edison_ysz_refs_report.md  -- the answer/report
...
def main():
    ...
    if answer:
        with open("edison_ysz_refs_report.md", "w") as f:
            f.write(str(answer))
        print("wrote edison_ysz_refs_report.md")
    else:
